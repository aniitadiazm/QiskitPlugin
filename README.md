SonarQube Custom Plugin Qiskit [![Build Status](https://travis-ci.org/SonarSource/sonar-custom-plugin-qiskit.svg?branch=7.x)](https://travis-ci.org/SonarSource/sonar-custom-plugin-qiskit)
==========

An qiskit SonarQube plugin compatible with SonarQube 9.x.
...
The JAR will be deployed to `target/sonar-qiskit-plugin-VERSION.jar`. Copy this to your SonarQube's `extensions/plugins/` directory, and re-start SonarQube.
...
* `npm start` to start a proxy server on port 3000 to debug your JS code.  
  *Note: This plugin must first be deployed and installed on your SonarQube instance, otherwise the extension paths will not be registered. See above under Back-end > Building*  
  ...
  ```
  PROXY_URL=https://sonarqube.qiskit.com npm start
  ```
...
This qiskit plugin uses [Webpack](https://webpack.js.org/) for building the final JavaScript. Whatever build system you choose to use, the final result *MUST* adhere to the following rules:

* 1 entry file *per extension page*.
* The name of each entry file must correspond to the `page_id` of the registered page (see `src/main/java/org/sonarsource/plugins/qiskit/web/MyPluginPageDefinition.java` and compare with the entry points in `conf/webpack/webpack.config.js`).
* Each entry file must be located in the resulting JAR's `static/` folder.

The building process should be included in your full packaging process. In this qiskit plugin, `mvn package` will call `npm run build` prior to finalizing the JAR package.
...
The pages are registered in `src/main/java/org/sonarsource/plugins/qiskit/web/MyPluginPageDefinition.java`, and their respective front-end source code is located in `src/main/js/`. These examples use different stacks to demonstrate different possibilities:

* React JS qiskit (recommended, SonarQube uses React 16):
  * `src/main/js/portfolio_page/`
  * `src/main/js/admin_page/`
* Backbone JS qiskit: `src/main/js/project_page/`
* Vanilla JS qiskit: `src/main/js/global_page/`
