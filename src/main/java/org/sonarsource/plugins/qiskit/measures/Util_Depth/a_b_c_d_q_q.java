...
public class a_b_c_d_q_q {
    ...
    public static int[] calculate(int[] depths, String linea, String[] lines) {
        ...
        if(part0.contains("[")) {
            ...
            part1 = parts[0];
            parts = part1.split("\\,");
            if(depths[Integer.valueOf(parts[4])] >= depths[Integer.valueOf(parts[5])]){
                max = depths[Integer.valueOf(parts[4])];
            } else{
                max = depths[Integer.valueOf(parts[5])];
            }
            depths[Integer.valueOf(parts[4])] = max + 1;
            depths[Integer.valueOf(parts[5])] = max + 1;
        }
        return depths;
    }
}